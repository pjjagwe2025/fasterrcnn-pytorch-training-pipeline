...
def parse_opt():
    ...
    parser.add_argument(
        ...
        help='number of epochs to wait for when mAP does not increase to \
              trigger early stopping. Set to 0 to disable early stopping',
        type=int
    )
    parser.add_argument(
        '--seed',
...
def main(args):
    ...
    if args['weights'] is not None:
        ...
        # FIXED: Handle both distributed (module.* prefix) and non-distributed checkpoints
        cls_score_key = None
        if 'roi_heads.box_predictor.cls_score.weight' in ckpt_state_dict:
            cls_score_key = 'roi_heads.box_predictor.cls_score.weight'
        elif 'module.roi_heads.box_predictor.cls_score.weight' in ckpt_state_dict:
            cls_score_key = 'module.roi_heads.box_predictor.cls_score.weight'
        else:
            raise KeyError("Could not find roi_heads.box_predictor.cls_score.weight in checkpoint")
        
        # Get the number of classes from the loaded checkpoint.
        old_classes = ckpt_state_dict[cls_score_key].shape[0]
        print(f"Checkpoint has {old_classes} classes")

        # Build the new model with number of classes same as checkpoint.
        build_model = create_model[args['model']]
        model = build_model(num_classes=old_classes)
        
        # FIXED: Handle loading state dict with or without module prefix
        if 'module.' in list(ckpt_state_dict.keys())[0]:
            # If checkpoint has module prefix, we need to handle it properly
            print("Checkpoint saved from distributed training, handling module prefix...")
            # Remove module prefix before loading (we'll add it back when wrapping with DDP)
            new_state_dict = {}
            ...
            # Checkpoint doesn't have module prefix
            print("Checkpoint saved from single GPU training...")
        ...
        if args['resume_training']:
            ...
            if 'epoch' in checkpoint and checkpoint['epoch']:
                start_epochs = checkpoint['epoch']
                print(f"Resuming from epoch {start_epochs}...")
            if 'train_loss_list' in checkpoint and checkpoint['train_loss_list']:
                print('Loading previous batch wise loss list...')
                train_loss_list = checkpoint['train_loss_list']
            else:
                print('No previous batch wise loss list found, starting fresh...')
            if 'train_loss_list_epoch' in checkpoint and checkpoint['train_loss_list_epoch']:
                print('Loading previous epoch wise loss list...')
                train_loss_list_epoch = checkpoint['train_loss_list_epoch']
            else:
                print('No previous epoch wise loss list found, starting fresh...')
            if 'val_map' in checkpoint and checkpoint['val_map']:
                print('Loading previous mAP list')
                val_map = checkpoint['val_map']
            else:
                print('No previous mAP list found, starting fresh...')
            if 'val_map_05' in checkpoint and checkpoint['val_map_05']:
                val_map_05 = checkpoint['val_map_05']
            else:
                print('No previous mAP@0.5 list found, starting fresh...')
    ...
    # Define the optimizer - matching YOLO/RetinaNet hyperparameters
    ...
    if args['resume_training'] and args['weights'] is not None: 
        ...
        if 'optimizer_state_dict' in checkpoint:
            ...
            print('No optimizer state dict found in checkpoint, using fresh optimizer...')

    # Use ReduceLROnPlateau scheduler to match RetinaNet
    ...
    # Only enable early stopping if patience > 0
    if args['patience'] > 0:
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        # Step the learning rate scheduler with validation mAP
        scheduler.step(stats[0])
